  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table :data="articleList" ref="" style="width: 100%" border>
        ...
        <el-table-column label="是否推荐" align="center" prop="level">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.level" :active-value="1" :inactive-value="0"
              @change="changeSwitch($event, scope.row.id, scope.row.level)">
            </el-switch>
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 修改推荐状态 
      changeSwitch(event, id, status) {
        let data = {
          "level": status
        }
        updateArticleLevel(id, data).then(resp => {
          if (resp.code === 200 && status === 1) {
            this.$message("推荐成功")
          } else {
            this.$message("关闭推荐")
          }
