<template>
  <div class="app-container">
    ...
    <el-dialog title="分配菜单" :visible.sync="dialogVisible" width="40%">
      <el-tree :data="treeMenuList" show-checkbox node-key="id" default-expand-all :default-checked-keys="expandedKeys"
        :props="defaultProps">
      </el-tree>
    </el-dialog>
  </div>
      ...
      allocateMenu(index, roleId) {
        getMenuListByRoleId(roleId).then(resp => {
          if (resp.code === 200) {
            ...
            let menuList1 = resp.data
            fetchTreeList().then(resp => {
              if (resp.code === 200) {
                this.treeMenuList = resp.data
                let treeMenuList1 = resp.data
              }
            });
          }
          console.log(this.menuList1.length, this.treeMenuList1.length)
          if (this.menuList1 != undefined && this.treeMenuList1 != undefined) {
            console.log(1111111111)
            for (let i = 0; i <= this.treeMenuList1.length; i++) {
              console.log(this.treeMenuList1[i].children)
              for (let j = 0; j <= this.treeMenuList1[i].children.length; j++) {
                console.log(2222222)
                for (let k = 0; k <= this.menuList1.length; k++) {
                  if (this.menuList1[i].children[j].id === this.menuList1[k].id) {
                    this.expandedKeys.push(this.menuList1[k].id)
                  }
                }
              }
            }
