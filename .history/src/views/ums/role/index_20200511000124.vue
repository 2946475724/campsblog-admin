    ...
    methods: {
      ...
      allocateMenu(index, roleId) {
        getMenuListByRoleId(roleId).then(resp => {
          ...
          console.log(this.menuList.length, this.treeMenuList.length)
          if (this.menuList != undefined && this.treeMenuList != undefined) {
            console.log(1111111111)
            for (let i = 0; i <= this.treeMenuList.length; i++) {
              console.log(this.treeMenuList[i].children)
              for (let j = 0; j <= this.treeMenuList[i].children.length; j++) {
                for (let k = 0; k <= this.menuList.length; k++) {
                  if (this.menuList[i].children[j].id === this.menuList[k].id) {
                    this.expandedKeys.push(this.menuList[k].id)
                  }
                }
              }
            }
