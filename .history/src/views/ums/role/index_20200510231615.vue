    ...
    methods: {
      ...
      allocateMenu(index, roleId) {
        getMenuListByRoleId(roleId).then(resp => {
          ...
          for (let i = 0; i <= this.treeMenuList.length; i++) { for (let j=0; j <=this.treeMenuList[i].children.length;
            j++) { for (let k=0; k <=this.menuList.length; k++) { if
            (this.menuList[i].children[j].id===this.menuList[k].id) { this.expandedKeys.push(this.menuList[k].id) } } }
            } }
        })
      },
      // 删除友情链接
