<template>
  <div class="app-container">
    ...
    <el-dialog :title="分配菜单" :visible.sync="dialogVisible" width="40%">
      <el-tree :data="treeMenuList" show-checkbox node-key="id" :default-expanded-keys="expandedKeys" default-expand-all
        ...
        for (let i = 0; i < this.treeMenuList.length; i++) {
          for (let j = 0; j < this.treeMenuList[i].children.length; j++) {
            for (let k = 0; k < this.menuList.length; k++) {
              if (this.menuList[i].children[j].title === this.menuList[k].title) {
